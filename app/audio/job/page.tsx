...
export default function AudioJobPage() {
...
function Page({ id }: { id: string }) {
	const { data, isFetched, isError } = useQuery(
		audioAPI[":id"].status.$post.queryOptions({
			param: { id },
						...
						<StepperContent value={"accepted"}>
							<Card>
								<CardHeader>
									...
									<Button asChild>
										<Link href={`/audio?id=${id}`}>Go to upload</Link>
									</Button>
								</CardHeader>
							</Card>
						</StepperContent>
...
function EmailStepContent({ id }: { id: string }) {
	...
	return (
		<CardContent>
			...
			<MutationButton
				api={audioAPI[":id"].email.$post.mutationOptions()}
				mutate={(mutate) => (
					<Button
						variant="outline"
						onClick={() =>
...
function DownloadStepContent({ id }: { id: string }) {
	const { data, isLoading, isError, error } = useQuery(
		audioAPI[":id"].download.$post.queryOptions({
			param: { id },
		}),
	);
