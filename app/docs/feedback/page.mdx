...
// why why why?
const outFileId = status.job_details[0].inputs.find((e) => e.file_name === filename)?.file_id;
...
#### Document Intellience Batch API
...
Surpise, Surpise. Most of the developer don't use python in production. Python is for ML researchers and Math nerds.
...
curl -X POST https://api.sarvam.ai/text-to-speech \
     -H "api-subscription-key: sk_t8uf1nb3_PmwE2eythwq2TjbPQt6mgBKj" \
     -H "Content-Type: application/json" \
     -d '{
...
```json
{
  "error": {
    ...
    "request_id": "20260615_cdd28e9a-8192-4f53-8cf2-225663754f7f"
  }
}
```
